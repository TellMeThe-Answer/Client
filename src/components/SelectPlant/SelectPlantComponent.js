                ...
                <div className="flex justify-center items-center bg-gray-50 w-full h-96 drop-shadow-md ...">
                    <div className="w-4/5 h-4/5 grid grid-cols-2 items-center justify-items-center">
                        {data.map((button) => (
                            ...
                            className="flex flex-col items-center justify-between h-36 w-36 transform transition-all duration-300 ease-in-out hover:scale-110"
                          >
                            <div className="h-full w-full p-5 flex flex-col items-center justify-center">
                              <img src={button.image} alt={button.name} className="mx-auto" />
                              <div className="mt-2">{button.name}</div>
                            </div>
                          </button>
                            // <button key={button.id} onClick={() => handleButtonClick(button)} className="flex flex-col items-center justify-between h-36 w-36">
                            //     <div className="h-full w-full p-5 flex flex-col items-center justify-center">
                            //         <img src={button.image} alt={button.name} className="mx-auto" />
                            //         <div className="mt-2">{button.name}</div>
                            //     </div>
